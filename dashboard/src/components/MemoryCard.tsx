...
export function MemoryCard({ memory, score }: { memory: Memory; score: number }) {
  return (
    <article className="rounded-lg border border-line bg-panel p-5 transition hover:border-signal/50">
      <div className="mb-3 flex items-center justify-between gap-3">
        <span className="font-sans text-[11px] font-medium uppercase tracking-[0.12em] text-muted">Semantic match</span>
        <ScoreBadge score={score} />
      </div>
      <p className="font-sans text-[15px] leading-7 text-ink">{memory.content}</p>
